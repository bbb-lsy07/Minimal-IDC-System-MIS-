...
# 创建 config.local.php 文件，使用临时数据库凭据
cat > "${INSTALL_DIR}/config.local.php" <<'PHPEOF'
<?php

declare(strict_types=1);

// Auto-generated by install.sh
// This file is ignored by git and contains local database credentials
...
# 替换占位符
sed -i "s|MisTemp123!|${DB_TEMP_PASS}|g" "${INSTALL_DIR}/config.local.php"
sed -i "s|APP_KEY_PLACEHOLDER|${APP_KEY}|g" "${INSTALL_DIR}/config.local.php"
sed -i "s|BASE_URL_PLACEHOLDER|${BASE_URL}|g" "${INSTALL_DIR}/config.local.php"

# 确保 config.local.php 权限正确
...
echo -e ""
echo -e "数据库配置 (已自动生成 config.local.php):"
...
echo -e ""
echo -e "${YELLOW}注意：${NC}"
echo -e "1. 数据库已自动配置，您可以直接访问应用"
echo -e "2. 如需重新初始化数据库表，请访问 web_install.php"
echo -e "3. 安装完成后，建议删除 web_install.php 文件以确保安全"
echo -e "4. 生产环境建议修改 config.local.php 中的数据库密码"